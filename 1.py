...
def get_download_base_dir() -> Path:
    """Return the base directory where downloaded videos should be stored."""
    config = load_local_config()
    custom_dir = (
        os.getenv("DOWNLOAD_DIR")
        or str(config.get("download_dir") or config.get("DOWNLOAD_DIR") or "")
    ).strip()
    if custom_dir:
        p = Path(custom_dir).resolve()
        p.mkdir(parents=True, exist_ok=True)
        return p
    default_dir = get_runtime_base_dir() / "src"
...
FFMPEG_BIN = "ffmpeg"
VIDEO_TTL_SECONDS = int(os.getenv("VIDEO_TTL_SECONDS", "0"))
...
def schedule_video_cleanup(filepath: Path, delay_seconds: int = VIDEO_TTL_SECONDS) -> None:
    """Delete generated video files after a fixed retention period (0 to disable)."""
    if delay_seconds <= 0:
        return
...
def generate_video_filename(
    ...
    """Generate a clean, standardized filename (series_id + tập) for the saved video in src/."""
    if custom_filename:
        name = str(custom_filename).strip()
        name = re.sub(r'[\\/*?:"<>|]', '_', name)
        if not name.lower().endswith(".mp4"):
            name += ".mp4"
        return name

    s_id = str(series_id or "").strip() if series_id else ""
    ep_str = str(episode or "").strip() if episode is not None else ""

    if s_id:
        s_id = re.sub(r'[\\/*?:"<>|]', '_', s_id)
        if ep_str:
            ep_match = re.search(r'\d+', ep_str)
            ep_num = ep_match.group(0) if ep_match else re.sub(r'[\\/*?:"<>|]', '_', ep_str)
            return f"{s_id}_tap_{ep_num}.mp4"
        else:
            return f"{s_id}.mp4"

    if video_id:
        v_id = re.sub(r'[\\/*?:"<>|]', '_', str(video_id).strip())
        return f"{v_id}.mp4"

    return f"video_{time.time_ns()}.mp4"
...
# In-memory thread-safe cache for video models: video_id -> (fallback_api, video_model, timestamp)
_VIDEO_MODEL_CACHE: Dict[str, Tuple[str, Dict[str, Any], float]] = {}
_VIDEO_MODEL_CACHE_LOCK = threading.Lock()
VIDEO_MODEL_CACHE_TTL = 1800.0  # 30 minutes


def get_cached_video_model(video_id: str) -> Optional[Tuple[str, Dict[str, Any]]]:
    """Retrieve fallback_api and video_model from cache if not expired."""
    now = time.time()
    with _VIDEO_MODEL_CACHE_LOCK:
        if video_id in _VIDEO_MODEL_CACHE:
            fallback_api, video_model, ts = _VIDEO_MODEL_CACHE[video_id]
            if now - ts < VIDEO_MODEL_CACHE_TTL:
                return fallback_api, video_model
            else:
                del _VIDEO_MODEL_CACHE[video_id]
    return None


def set_cached_video_model(video_id: str, fallback_api: str, video_model: Dict[str, Any]) -> None:
    """Store fallback_api and video_model into cache."""
    now = time.time()
    with _VIDEO_MODEL_CACHE_LOCK:
        _VIDEO_MODEL_CACHE[video_id] = (fallback_api, video_model, now)


def resolve_batch_video_models(
    video_ids: List[str],
    batch_size: int = 20,
    max_retries: int = 3,
) -> Dict[str, Any]:
    """
    Resolve video models for a batch of video_ids in bulk using 1 Liushen signature per chunk.
    Caches the resulting (fallback_api, video_model) for fast concurrent downloading.
    """
    cleaned_ids = [str(v).strip() for v in video_ids if str(v).strip()]
    if not cleaned_ids:
        return {"ok": True, "total": 0, "cached": 0, "resolved": 0, "results": {}}
    ...
    for vid in cleaned_ids:
        cached = get_cached_video_model(vid)
        if cached:
            results[vid] = {"ok": True, "cached": True}
        else:
            needed_ids.append(vid)

    cached_count = len(results)
    resolved_count = 0

    # 2. Chunk needed IDs into batches
        ...
        for attempt in range(max_retries):
            try:
                ...
                if not isinstance(data, dict) or "data" not in data:
                    raise ValueError("video_model response missing data field")

                data_map = data.get("data", {})
                for vid in chunk:
                    if vid in data_map:
                        try:
                            fallback_str, video_model = extract_fallback_api(data, vid)
                            set_cached_video_model(vid, fallback_str, video_model)
                            results[vid] = {"ok": True, "cached": False}
                            resolved_count += 1
                        except Exception as parse_err:
                            results[vid] = {"ok": False, "error": str(parse_err)}
                    else:
                        results[vid] = {"ok": False, "error": "vid not in response data map"}
        ...
        if not chunk_success:
            for vid in chunk:
                if vid not in results:
                    results[vid] = {"ok": False, "error": str(last_err)}
...
def resolve_stream_info(
    ...
    """
    Resolve video stream info (remote CDN URL, CENC decryption key, metadata)
    without downloading or writing local files.
    """
    video_id = str(video_id).strip()
    device_keys = get_device_keys()

    cached = get_cached_video_model(video_id)
    if cached:
        fallback_api, video_model = cached
    else:
        target_url = build_video_model_url(
            device_keys["device_id"], device_keys["install_id"]
        )
        ...
        set_cached_video_model(video_id, fallback_api, video_model)
            ...
            if not video_data:
                ...
                if isinstance(video_list, dict):
                    ...
                    if best_key and best_item:
                        ...
                        duration_val = first_non_empty(video_model.get("duration"), video_data.get("duration"))
                        try:
                            duration = float(duration_val or 0.0)
                        except Exception:
                            duration = 0.0

                        w_val = first_non_empty(best_item.get("vwidth"), best_item.get("width"))
                        h_val = first_non_empty(best_item.get("vheight"), best_item.get("height"))
                        try:
                            w = int(float(w_val))
                        except Exception:
                            w = 0
                        ...
                        return {
                            "video_id": video_id,
                            "url": real_main_url,
                            "content_key_hex": content_key.hex() if content_key else "",
                            "duration": duration,
                            "width": w,
                            "height": h,
                            "has_audio": True,
    ...
    raise Exception(f"Failed to resolve stream info for video_id={video_id}: {last_err}")


def resolve_batch_stream_infos(
    video_ids: List[str],
    max_workers: int = 10,
) -> List[Dict[str, Any]]:
    """
    Resolve stream info for a list of video_ids concurrently.
    Pre-populates video_model cache via batch resolve, then fetches fallback stream URLs in parallel.
    """
    cleaned_ids = [str(v).strip() for v in video_ids if str(v).strip()]
    if not cleaned_ids:
        return []

    resolve_batch_video_models(cleaned_ids, batch_size=30)
    ...
    def _worker(vid: str):
        try:
            ...
            results_map[vid] = info
        except Exception as exc:
            print(f"[resolve_batch_stream_infos] failed vid={vid} error={exc}")

    with ThreadPoolExecutor(max_workers=min(max_workers, len(cleaned_ids))) as executor:
        list(executor.map(_worker, cleaned_ids))

    out = []
    for vid in cleaned_ids:
        if vid in results_map:
            out.append(results_map[vid])
        else:
            raise RuntimeError(f"Không thể giải mã luồng video cho tập {vid}")
    return out
...
def resolve_video_url(
    ...
    """
    Full flow (with retries & cache):
      1. get device_keys
      2. check cache or POST multi_video_model
    ...
    if request is not None and not save_dir:
        try:
            if hasattr(request, "args"):
                save_dir = request.args.get("save_dir") or (request.form.get("save_dir") if hasattr(request, "form") else None)
        except Exception:
            pass

    # Step A: Check if (fallback_api, video_model) is already cached (e.g. from batch resolve)
    cached = get_cached_video_model(video_id)
    if cached:
        fallback_api, video_model = cached
        try:
            return download_and_decrypt_video(
                request,
                fallback_api,
                video_model,
                device_keys,
            ...
            print(f"[cache_hit_failed] vid={video_id} error={exc}, falling back to fresh resolve...")

    # Step B: Fresh resolve with retries
    last_err: Optional[Exception] = None
...
def download_and_decrypt_video(
    ...
    if request is not None:
        try:
            if hasattr(request, "args"):
                if not series_id:
                    series_id = request.args.get("series_id") or (request.form.get("series_id") if hasattr(request, "form") else None)
                if episode is None:
                    episode = request.args.get("episode") or request.args.get("ep") or (request.form.get("episode") if hasattr(request, "form") else None)
                if not filename:
                    filename = request.args.get("filename") or (request.form.get("filename") if hasattr(request, "form") else None)
                if not save_dir:
                    save_dir = request.args.get("save_dir") or (request.form.get("save_dir") if hasattr(request, "form") else None)
        except Exception:
            pass

    target_filename = filename or generate_video_filename(video_id, series_id=series_id, episode=episode)
                        ...
                        if best_key and best_item:
                            ...
                            if raw_main_url:
                                ...
                                try:
                                    ...
                                    print(f"[download_decrypt_and_serve] error={exc}")
...
def stream_copy_video_with_ffmpeg(
    ...
    if save_dir and str(save_dir).strip():
        target_dir = Path(str(save_dir).strip()).resolve()
    else:
        target_dir = get_download_base_dir()
    ...
    print(f"[timing] stream_copy_seconds={stream_seconds:.3f} file={filepath}")
    schedule_video_cleanup(filepath)
    domain = request_or_domain if isinstance(request_or_domain, str) and request_or_domain.startswith("http") else get_current_domain(request_or_domain)
    
    src_dir = get_runtime_base_dir() / "src"
    try:
        if filepath.resolve().is_relative_to(src_dir.resolve()):
            rel_path = filepath.resolve().relative_to(src_dir.resolve()).as_posix()
            return f"{domain}/src/{rel_path}"
    except Exception:
        pass

    return f"{domain}/api/video_file?path={quote(str(filepath))}"
...
def save_video_bytes(request, decrypted_data: bytes, filename: Optional[str] = None) -> str:
